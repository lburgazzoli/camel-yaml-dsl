...
import com.github.lburgazzoli.camel.dsl.yaml.common.exception.UnsupportedFieldException;
...
public class BeansDeserializer implements ConstructNode {
    @Override
    public Object construct(Node node) {
        final SequenceNode sn = YamlDeserializerSupport.asSequenceNode(node);
    ...
    private static CamelContextCustomizer createCustomizer(Node node) {
        ...
        for (NodeTuple tuple : bn.getValue()) {
            ...
            switch (key) {
                case "name":
                    name = YamlDeserializerSupport.asText(val);
                    break;
                case "type":
                    type = YamlDeserializerSupport.asText(val);
