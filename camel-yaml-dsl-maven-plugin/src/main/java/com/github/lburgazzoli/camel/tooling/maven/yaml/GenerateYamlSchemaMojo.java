...
public class GenerateYamlSchemaMojo extends GenerateYamlSupportMojo {
    ...
    public void execute() throws MojoFailureException {
        ...
        step = definitions.with("step");
        step.put("type", "object");
        step.put("maxProperties", 1);
        ...
        annotated(YAML_TYPE_ANNOTATION)
            ...
            .forEach(ci -> {
                annotationValue(ci, YAML_TYPE_ANNOTATION, "types")
                    ...
                    .stream()
                    .flatMap(Stream::of)
                    .forEach(item -> types.putIfAbsent(item, ci));
        ...
        for (Map.Entry<String, ClassInfo> entry : types.entrySet()) {
            ...
            if (hasAnnotation(entry.getValue(), YAML_IN_ANNOTATION)) {
                nodes.forEach(node -> {
                    items.with("properties").putObject(node).put("$ref", "#/items/definitions/" + entry.getKey());
                });
            } else {
                ...
                if (extendsType(info, PROCESSOR_DEFINITION_CLASS)) {
                    nodes.forEach(node -> {
                        step.with("properties").putObject(node).put("$ref", "#/items/definitions/" + entry.getKey());
                    });
                }
            }
    ...
    private void generate(String type, ClassInfo info) {
        ...
        properties.sort(Comparator.comparing(property -> annotationValue(property, "name").map(AnnotationValue::asString).orElse("")));
        ...
        for (AnnotationInstance property: properties) {
            ...
            final boolean propertyRequired = annotationValue(property, "required").map(AnnotationValue::asBoolean).orElse(false);
            ...
            if (propertyType.startsWith("object:")) {
                ...
                if (arrayType.contains(".")) {
                    ...
                    //TODO: how this can be auto generated ?
                    if ("step".equals(arrayType)) {
                        objectDefinition
                            .with("properties")
                            .with(propertyName)
                            .put("type", "array")
                            .with("items").put("$ref", "#/items/definitions/step");
                    } else {
                        objectDefinition
                            .with("properties")
                            .with(propertyName)
                ...
                for (String enumValue : property.value("values").asStringArray()) {
                    objectDefinition
                        .with("properties")
                        .with(propertyName)
                        .withArray("enum")
        ...
        annotationValue(info, YAML_TYPE_ANNOTATION, "properties")
            ...
            .forEach(property -> {
                ...
                if (ObjectHelper.isEmpty(propertyName) || ObjectHelper.isEmpty(propertyType)) {
                    getLog().warn("Missing name or type for property + " + property + " on type " + info.name().toString());
                    return;
                }
